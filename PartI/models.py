...
#         super().__init__()
#         self.delta = delta
#         self.k_center = tf.math.log(1 / (1 - delta)) / 2
...
#     def call(self, z, k):
#         x = self.concat([z[:,None], tf.math.log(k[:,None]) / 40 - self.k_center])
#         x = self.dense1(x)
#         x = self.dense2(x)
#         x = self.dense3(x)
...
class Actor(tf.keras.Model):
    ...
    def call(self, z, k):
        ...
        x = self.dense1(x)
        x = self.dense2(x)
        x = tf.squeeze(self.dense3(x), axis=-1)
        # return (tf.clip_by_value(x, 0, 100) - (1 - self.delta)) * k
        return tf.exp(x) - (1 - self.delta) * k
...
class Value(tf.keras.Model):
    ...
    def call(self, z, k):
        ...
        x = self.dense1(x)
        x = self.dense2(x)
        return tf.squeeze(self.dense3(x), axis=-1)